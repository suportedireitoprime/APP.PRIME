  ...
  SELECT * FROM (
    ...
    SELECT * FROM (
      SELECT DISTINCT ON (s.user_id)
        s.user_id::text AS key, s.user_id,
        COALESCE(p.display_name, split_part(u.email,'@',1), 'Usuário')::text AS title,
        u.email::text AS email, s.initial_route::text AS subtitle, MAX(s.started_at) OVER (PARTITION BY s.user_id) AS at,
        COUNT(*) OVER (PARTITION BY s.user_id)::int AS acessos,
        COALESCE(u.raw_user_meta_data->>'avatar_url', u.raw_user_meta_data->>'picture')::text AS avatar_url,
    ...
    SELECT s.id::text, s.user_id,
      COALESCE(p.display_name, split_part(u.email,'@',1), 'Assinatura Asaas')::text,
      u.email::text,
      ('Asaas · ' || COALESCE(s.plano,'—') || ' · ' || COALESCE(s.status::text,''))::text,
      s.updated_at, NULL::int,
      COALESCE(u.raw_user_meta_data->>'avatar_url', u.raw_user_meta_data->>'picture')::text AS avatar_url,
...
s.rpc('admin_sql', {query: sql}).then(r => console.log('OK', r));
