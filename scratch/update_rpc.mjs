...
BEGIN
  ...
  ELSIF _acao = 'downgrade' THEN
    UPDATE public.profiles SET is_premium = false, updated_at = now() WHERE id = _user_id;
    UPDATE public.asaas_subscriptions SET status = 'CANCELED', updated_at = now() WHERE user_id = _user_id;
    UPDATE public.legacy_subscribers SET status = 'inactive' WHERE claimed_user_id = _user_id;
    UPDATE public.play_subscriptions SET status = 'CANCELED' WHERE user_id = _user_id;
    RETURN jsonb_build_object('success', true, 'acao', 'downgrade');
...
`;
  ...
  console.log('admin-sql result:', data, error);
}
run();
