...
CREATE POLICY "apresentacoes publicadas sao publicas" ON public.apresentacoes_narradas FOR SELECT USING (publicada = true OR public.is_admin_user((select auth.uid())));
CREATE POLICY "admins gerenciam apresentacoes" ON public.apresentacoes_narradas FOR ALL TO authenticated USING (public.is_admin_user((select auth.uid()))) WITH CHECK (public.is_admin_user((select auth.uid())));
...
CREATE POLICY "slides visiveis com a apresentacao" ON public.apresentacao_slides FOR SELECT USING (EXISTS (SELECT 1 FROM public.apresentacoes_narradas a WHERE a.id = apresentacao_id AND (a.publicada = true OR public.is_admin_user((select auth.uid())))));
CREATE POLICY "admins gerenciam slides" ON public.apresentacao_slides FOR ALL TO authenticated USING (public.is_admin_user((select auth.uid()))) WITH CHECK (public.is_admin_user((select auth.uid())));
...
CREATE POLICY "usuario gerencia seus likes" ON public.apresentacao_likes FOR INSERT TO authenticated WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "usuario remove seus likes" ON public.apresentacao_likes FOR DELETE TO authenticated USING ((select auth.uid()) = user_id);
...
CREATE POLICY "usuario ve seus favoritos" ON public.apresentacao_favoritos FOR SELECT TO authenticated USING ((select auth.uid()) = user_id);
CREATE POLICY "usuario cria seus favoritos" ON public.apresentacao_favoritos FOR INSERT TO authenticated WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "usuario remove seus favoritos" ON public.apresentacao_favoritos FOR DELETE TO authenticated USING ((select auth.uid()) = user_id);
...
CREATE POLICY "usuario comenta" ON public.apresentacao_comentarios FOR INSERT TO authenticated WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "usuario edita seu comentario" ON public.apresentacao_comentarios FOR UPDATE TO authenticated USING ((select auth.uid()) = user_id) WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "usuario ou admin apaga comentario" ON public.apresentacao_comentarios FOR DELETE TO authenticated USING ((select auth.uid()) = user_id OR public.is_admin_user((select auth.uid())));
