...
CREATE POLICY "Boletim tipos escrita admin" ON storage.objects FOR ALL
  USING (bucket_id = 'boletim-tipos' AND public.is_admin_user((select auth.uid())))
  WITH CHECK (bucket_id = 'boletim-tipos' AND public.is_admin_user((select auth.uid())));
...
CREATE POLICY "Boletins audio escrita service" ON storage.objects FOR ALL
  USING (bucket_id = 'boletins-audio' AND public.is_admin_user((select auth.uid())))
  WITH CHECK (bucket_id = 'boletins-audio' AND public.is_admin_user((select auth.uid())));
...
CREATE POLICY "Boletins video escrita admin" ON storage.objects FOR ALL
  USING (bucket_id = 'boletins-video' AND public.is_admin_user((select auth.uid())))
  WITH CHECK (bucket_id = 'boletins-video' AND public.is_admin_user((select auth.uid())));
