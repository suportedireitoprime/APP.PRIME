...
CREATE POLICY "Users read own audio"
  ...
  USING (bucket_id = 'anotacoes-audio' AND (storage.foldername(name))[1] = (select auth.uid())::text);

CREATE POLICY "Users upload own audio"
  ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'anotacoes-audio' AND (storage.foldername(name))[1] = (select auth.uid())::text);
