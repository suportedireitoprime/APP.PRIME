CREATE POLICY "narracoes conteudo leitura admin"
  ...
  USING (bucket_id = 'narracoes-conteudo' AND public.is_admin_user((select auth.uid())));

CREATE POLICY "narracoes conteudo insere admin"
  ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'narracoes-conteudo' AND public.is_admin_user((select auth.uid())));
