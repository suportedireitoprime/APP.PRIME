  ...
  WHERE q.ativo = true
    ...
    AND (
      ...
      OR NOT EXISTS (
        ...
        WHERE r.questao_id = q.id AND r.user_id = (select auth.uid()) AND r.acertou = true
      )
    )
  ORDER BY random()
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE q.ativo = true
    ...
    AND EXISTS (
      SELECT 1 FROM public.questoes_respostas r
      WHERE r.questao_id = q.id AND r.user_id = (select auth.uid()) AND r.acertou = false
    )
    AND NOT EXISTS (
      SELECT 1 FROM public.questoes_respostas r2
      WHERE r2.questao_id = q.id AND r2.user_id = (select auth.uid()) AND r2.acertou = true
        AND r2.created_at > (
          SELECT MAX(r3.created_at) FROM public.questoes_respostas r3
          WHERE r3.questao_id = q.id AND r3.user_id = (select auth.uid()) AND r3.acertou = false
        )
    )
  ORDER BY random()
  LIMIT GREATEST(1, LEAST(_limit, 100));
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT jsonb_build_object(
    'total', COALESCE((SELECT COUNT(*) FROM public.questoes_respostas WHERE user_id = (select auth.uid())), 0),
    'acertos', COALESCE((SELECT COUNT(*) FROM public.questoes_respostas WHERE user_id = (select auth.uid()) AND acertou), 0),
    'hoje', COALESCE((SELECT COUNT(*) FROM public.questoes_respostas WHERE user_id = (select auth.uid()) AND created_at::date = CURRENT_DATE), 0),
    'simulados', COALESCE((SELECT COUNT(*) FROM public.questoes_simulados WHERE user_id = (select auth.uid()) AND status = 'concluido'), 0),
    'por_area', COALESCE((
