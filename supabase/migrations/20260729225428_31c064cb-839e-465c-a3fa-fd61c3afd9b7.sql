DO $$
DECLARE
  v_md text; v_pages text[]; v_txt text; v_n int;
  v_page_txt text[]; caps jsonb := '[]'::jsonb;
  conteudo text; parte text; final_md text := ''; prelim text := '';
  i int; p int;
  titulos text[] := ARRAY[
    'Introdução',
...
BEGIN
  SELECT conteudo_md_refinado INTO v_md FROM biblioteca_leitura_nativa
  ...
  FOR i IN 1..array_length(v_pages,1) LOOP
    IF v_pages[i] ~ '^[0-9]+ -->' THEN
      ...
      v_txt := replace(v_txt, '<!-- capa-capitulo -->', '');
      -- remove blocos de capa: "# Capítulo N" + "## Título" + linha de metadados
      v_txt := regexp_replace(v_txt, '(^|\n)#{1,6}\s*Cap[íi]tulo\s+\d+\s*\n', '\1', 'g');
      v_txt := regexp_replace(v_txt, '(^|\n)\*\d+\s+p[áa]ginas\s*·[^\n]*\*\s*\n?', '\1', 'g');
      FOR p IN 1..array_length(titulos,1) LOOP
        v_txt := replace(v_txt, '## ' || titulos[p] || E'\n', '');
      END LOOP;
      -- remove "## Titulo" órfão logo após capa removida (linha isolada seguida de metadados já retirados)
      v_txt := regexp_replace(v_txt, '(^|\n)-{3,}\s*(\n|$)', '\1', 'g');
      v_txt := regexp_replace(v_txt, '\n{3,}', E'\n\n', 'g');
      IF v_n BETWEEN 1 AND 99 THEN v_page_txt[v_n] := btrim(v_txt); END IF;
    END IF;
  END LOOP;
  ...
  FOR i IN 1..array_length(titulos,1) LOOP
    ...
    caps := caps || jsonb_build_array(jsonb_build_object(
      'numero', i, 'titulo', titulos[i], 'capa_md', capa,
      'paginas', jsonb_build_array(inicios[i], fins[i]), 'conteudo_md', conteudo));
    final_md := final_md || CASE WHEN final_md='' THEN '' ELSE E'\n\n---\n\n' END || capa || E'\n\n' || conteudo;
  END LOOP;

  UPDATE biblioteca_leitura_nativa
  SET conteudo_md_refinado = final_md, capitulos_json = caps,
      preliminares_md = NULLIF(prelim,''), refino_updated_at = now()
  WHERE livro_id='128' AND livro_tabela='biblioteca_classicos';
END $$;