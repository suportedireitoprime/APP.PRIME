...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Rede de segurança de métrica: carimba a campanha na própria URL de destino.
    // Se o caminho nativo de "abertura" falhar em algum aparelho, o app converte
    // `_pc` em um evento `opened` no boot (o push-track deduplica por install_id).
    if (payload.url) {
      let destUrl = payload.url;
      if (payload.campaign_id && !/[?&]_pc=/.test(destUrl)) {
        destUrl += (destUrl.includes("?") ? "&" : "?") + `_pc=${payload.campaign_id}`;
      }
      dataPayload.url = destUrl;
    }
      ...
      const results = await Promise.all(batch.map(async (r) => {
        ...
        if (isWeb) {
          message.webpush = {
            ...
            fcm_options: dataPayload.url ? { link: dataPayload.url } : undefined,
          };
        } else {
