...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "__healthcheck") {
      return json({
        ok: true,
        function: "biblioteca-ocr-mistral",
    ...
    if (action === "gerar_flashcards") return await handleGerarFlashcardsAula(req, raw);
    if (action === "gerar_questoes") return await handleGerarQuestoesAula(req, raw);
    if (action === "refino") return await handleRefino(raw);
    if (action === "analisar_sumario") return await handleAnalisarSumarioLivro(req, raw);
    if (action === "worker") return await handleWorker(raw?.livro_tabela ?? null, raw?.imediato === true, raw?.background === true);
    ...
    if (raw?.inline !== true) {
      ...
      // Egress: nunca trazer conteudo_md/conteudo_md_refinado aqui — esta rota é
      // chamada toda vez que o leitor abre. Só colunas de controle.
      const { data: existing } = await supabase
        .from("biblioteca_leitura_nativa")
        .select("status,refino_status,updated_at")
        .eq("livro_tabela", livro_tabela)
        .eq("livro_id", livro_id)
        .maybeSingle();

      const leituraRefinadaPronta =
        existing?.status === "pronto" && existing?.refino_status === "pronto";
      ...
      // Existe OCR bruto? Descobrimos com um count (head=true) — zero bytes de texto.
      let temOcrBruto = false;
      if (existing && !refinoVivo && existing.refino_status !== "pronto" && !force) {
        const { count } = await supabase
          .from("biblioteca_leitura_nativa")
          .select("livro_id", { count: "exact", head: true })
          .eq("livro_tabela", livro_tabela)
          .eq("livro_id", livro_id)
          .not("conteudo_md", "is", null);
        temOcrBruto = (count ?? 0) > 0;
      }
      ...
      const precisaApenasRefino = temOcrBruto;
      ...
      if (!emAndamentoRecente) {
        ...
        // Se o livro já tem conteúdo refinado salvo, uma falha de reprocessamento
        // NÃO pode rebaixá-lo para "erro" — ele continua legível para o usuário.
        const marcarFalha = async (detalhe: string) => {
          const { count } = await supabase
            .from("biblioteca_leitura_nativa")
            .select("livro_id", { count: "exact", head: true })
            .eq("livro_tabela", livro_tabela)
            .eq("livro_id", livro_id)
            .not("conteudo_md_refinado", "is", null);
          const temConteudo = (count ?? 0) > 0;
          await supabase
            .from("biblioteca_leitura_nativa")
            .update(
              temConteudo
                ? { status: "pronto", etapa: "Finalizado", progresso: 6, total_etapas: 6, erro_detalhe: null }
                : { status: "erro", erro_detalhe: detalhe.slice(0, 500) }
            )
            .eq("livro_tabela", livro_tabela)
            .eq("livro_id", livro_id);
        };
    ...
    // Cache
    // Egress: só as colunas de controle. `select("*")` traria o markdown inteiro.
...
const ANALISAR_SUMARIO_SYSTEM_PROMPT = `Você é um professor de Direito planejando um CURSO em vídeo-aula a partir de um LIVRO jurídico.
...
async function handleAnalisarSumarioLivro(req: Request, body: any) {
  const currentGeminiKey = geminiKey();
  if (!currentGeminiKey) {
    return json({ error: "GEMINI_API_KEY ausente. Salve sua chave do Gemini nos secrets do projeto." }, 500);
  }
  if (!SUPABASE_ANON_KEY) return json({ error: "SUPABASE_ANON_KEY ausente" }, 500);
  ...
  const livroNativaId = String(body?.livro_nativa_id || "").trim();
  const areaId = body?.area_id;
  if (!livroNativaId) return json({ error: "livro_nativa_id obrigatório" }, 400);

  const admin = createClient(SUPABASE_URL, SERVICE_ROLE);
  const { data: livro, error: livroErr } = await admin
    .from("biblioteca_leitura_nativa")
    .select("id, livro_id, livro_tabela, sumario_json, capitulos_json, status, refino_status")
    .eq("id", livroNativaId)
    .maybeSingle();
  if (livroErr || !livro) return json({ error: "livro OCR não encontrado" }, 404);

  const { data: conteudoRow } = await admin
    .from("biblioteca_leitura_nativa")
    .select("conteudo_md_refinado, conteudo_md")
    .eq("id", livroNativaId)
    .maybeSingle();
  const conteudoBase = String(conteudoRow?.conteudo_md_refinado || conteudoRow?.conteudo_md || "");
  ...
  const userContent = [
    ...
    conteudoBase.slice(0, 45000),
  ].join("\n");

  let aulas: any[] = [];
  let raw = "";
  for (let attempt = 0; attempt < 3; attempt++) {
    raw = await chatGemini(MODEL_FAST, ANALISAR_SUMARIO_SYSTEM_PROMPT, userContent, true);
    const parsed = parseJsonObject(raw);
    const arr = Array.isArray(parsed?.aulas) ? parsed.aulas : [];
    if (arr.length > 0) {
      aulas = arr;
      break;
    }
  ...
  const resolvedAreaId = await resolveAreaIdForLivro(admin, areaId, livro);
  ...
  return json({ ok: true, total: inseridas?.length ?? 0, aulas: inseridas, provider: GEMINI_PROVIDER, model: MODEL_FAST });
}
...
async function resolveAreaIdForLivro(admin: any, bodyAreaId: unknown, livro: any): Promise<string | null> {
...
async function handleRefino(body: RefinoBody) {
  ...
  const supabase = createClient(SUPABASE_URL, SERVICE_ROLE);
  // Egress: NÃO usar select("*") aqui — traz conteudo_md e conteudo_md_refinado
  // (megabytes cada) só para checar status. Primeiro lemos o cabeçalho leve.
  const { data: row, error } = await supabase
    .from("biblioteca_leitura_nativa").select("refino_status,status,sumario_json")
    .eq("livro_tabela", livro_tabela).eq("livro_id", livro_id).maybeSingle();
  if (error) throw error;
  if (!row) return json({ status: "sem_ocr", error: "Leitura nativa não encontrada. Rode o OCR primeiro." }, 200);
  ...
  if (!baseMd) {
    ...
    // Um 400 vira "non-2xx status code" no supabase-js e perde a mensagem.
    return json({ status: "sem_ocr", error: "Sem conteúdo para refinar. Rode o OCR deste livro primeiro." }, 200);
  }
...
async function chatGemini(model: string, system: string, user: string, jsonMode = false) {
  ...
  const resp = await fetchWithRetry(
    `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(currentGeminiKey)}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- NUMERAÇÃO ROMANA: muitos livros abrem o capítulo com um algarismo romano SOZINHO numa linha ("I", "II", "III", "IV") e o título logo abaixo, em caixa alta e possivelmente quebrado em várias linhas com hifenização. Trate SEMPRE esse par (romano + bloco em caixa alta) como INÍCIO DE CAPÍTULO na página onde ele aparece. O mesmo vale para arábicos isolados ("2", "3.").
- Ao montar o título, junte as linhas quebradas, desfaça a hifenização ("OBSTÁ-CULOS" → "obstáculos"), remova letras soltas duplicadas do OCR ("O O SIGNIFICADO" → "O significado") e devolva em Capitalização de Título — nunca tudo em maiúsculas e nunca com o algarismo romano no início.
- Não perca capítulos intermediários: se identificou o capítulo I e o III, procure o II entre eles antes de concluir.`;
...
ABERTURA DE CAPÍTULO (regra crítica — erre para menos, nunca ignore):
- Muitos livros abrem o capítulo com o NÚMERO sozinho numa linha (algarismo romano "II", "IV" ou arábico "2") e, logo abaixo, o TÍTULO em caixa alta, às vezes quebrado em várias linhas e com hifenização ("OBSTÁ-\\nCULOS").
- Nesse caso: JUNTE o número + todas as linhas do título numa ÚNICA linha, desfaça a hifenização, converta para "Capitalização de Título" (não deixe tudo em maiúsculas) e devolva como "## Título" (sem repetir o número romano/arábico dentro do texto).
  Exemplo: linhas "II" / "O O SIGNIFICADO DE UM DIREITO AO ACESSO / EFETIVO À JUSTIÇA: OS OBSTÁ- / CULOS A SEREM TRANSPOSTOS" → "## O Significado de um Direito ao Acesso Efetivo à Justiça: os Obstáculos a Serem Transpostos"
- Remova letras/símbolos soltos que o OCR cria antes do título (capitular decorativa duplicada, como o "O O" acima, "|", "•", "l").
- NUNCA deixe o número do capítulo isolado numa linha própria, nem deixe o título do capítulo como texto corrido do parágrafo.
- Se o título bater com um item do sumário do livro no CONTEXTO acima, use a grafia do sumário.
...
- **JAMAIS** deixar o NÚMERO DE PÁGINA SOLTO do rodapé/cabeçalho. Se a página termina (ou começa) com uma linha contendo apenas um número — "7", "128", "— 27 —", "· 27 ·", "[27]", "(27)", "7 |", "| 7" — REMOVA essa linha por completo. Isso vale mesmo que o número esteja isolado no fim do texto, após o último parágrafo. Nunca converta esse número em parágrafo, título, lista ou nota.
...
// ============================================================
async function handleWorker(livroTabela: string | null = null, imediato = false, background = false) {
  const supabase = createClient(SUPABASE_URL, SERVICE_ROLE);
  const MAX_JOBS = 3, MAX_TENT = 3;
  const results: any[] = [];

  const pegarProximoJob = async () => {
    let q = supabase
      .from("biblioteca_leitura_jobs").select("*")
      .eq("status", "agendado");
    // Execução manual ("Rodar fila agora") ignora o agendamento futuro.
    if (!imediato) q = q.lte("scheduled_for", new Date().toISOString());
  ...
  const processarJob = async (job: any) => {
    try {
      // Nunca reprocessar um livro que já está lido e refinado: reprocessar com
      // force derruba um livro pronto para "erro" quando o OCR falha.
      const { data: jaFeito } = await supabase
        .from("biblioteca_leitura_nativa")
        .select("status,refino_status")
        .eq("livro_tabela", job.livro_tabela)
        .eq("livro_id", job.livro_id)
        .maybeSingle();
      if (jaFeito?.status === "pronto" && jaFeito?.refino_status === "pronto") {
        await supabase.from("biblioteca_leitura_jobs")
          .update({ status: "ok", finished_at: new Date().toISOString(), erro: null })
          .eq("id", job.id);
        return { id: job.id, livro_id: job.livro_id, livro_tabela: job.livro_tabela, titulo: job.titulo, status: "ok", pulado: true };
      }
      ...
      await supabase.from("biblioteca_leitura_jobs")
        .update({ status: "ok", finished_at: new Date().toISOString(), erro: null })
        .eq("id", job.id);
      return { id: job.id, livro_id: job.livro_id, livro_tabela: job.livro_tabela, titulo: job.titulo, status: "ok" };
    } catch (e: any) {
      const retry = job.tentativas < MAX_TENT;
      ...
      return { id: job.id, livro_id: job.livro_id, livro_tabela: job.livro_tabela, titulo: job.titulo, status: retry ? "agendado" : "erro" };
    }
  };

  const escopo = livroTabela ?? "todas";
  const filaLigada = async () => {
    const { data } = await supabase
      .from("biblioteca_leitura_fila_estado")
      .select("rodando").eq("escopo", escopo).maybeSingle();
    return data?.rodando === true;
  };
  const desligarFila = async () => {
    await supabase.from("biblioteca_leitura_fila_estado")
      .upsert({ escopo, rodando: false, atualizado_em: new Date().toISOString() }, { onConflict: "escopo" });
  };
  ...
  if (background) {
    ...
    if (!job) {
      await desligarFila();
      return json({ processed: 0, results: [], fila: "vazia" });
    }
    const tentativas = (job.tentativas ?? 0) + 1;
    await supabase.from("biblioteca_leitura_jobs")
      .update({ status: "rodando", started_at: new Date().toISOString(), finished_at: null, erro: null, tentativas })
      .eq("id", job.id);
    const runningJob = { ...job, status: "rodando", tentativas };
    // Encadeia no servidor: ao terminar este livro, se a fila continuar ligada,
    // dispara o próximo automaticamente — mesmo com o app fechado.
    const task = (async () => {
      await processarJob(runningJob);
      if (await filaLigada()) {
        await invokeSelf({ action: "worker", imediato: true, background: true, ...(livroTabela ? { livro_tabela: livroTabela } : {}) });
      }
    })();
    // @ts-ignore - EdgeRuntime é global no Supabase Edge Functions
    if (typeof EdgeRuntime !== "undefined" && EdgeRuntime?.waitUntil) {
    ...
    return json({ processed: 1, results: [{ id: job.id, livro_id: job.livro_id, livro_tabela: job.livro_tabela, titulo: job.titulo, status: "rodando" }] });
  }
  ...
  for (let i = 0; i < MAX_JOBS; i++) {
    ...
    await supabase.from("biblioteca_leitura_jobs")
      ...
      .eq("id", job.id);
    results.push(await processarJob({ ...job, tentativas: job.tentativas + 1 }));
  }
