...
// v5 — leitura nativa usa obrigatoriamente a chave própria do Gemini, sem consumir créditos Lovable.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[gerar-aula-livro-v2] provedor=gemini-proprio modelo=${MODEL}`);
    ...
    console.log(`[gerar-aula-livro-v2] recorte=${modoRecorte}; chars=${conteudoLivro.length}; livro_chars=${mdCompleto.length}`);
    ...
    for (let attempt = 0; attempt < MODELS.length * 2; attempt++) {
      ...
      const aiRes = await fetch("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", {
        method: "POST",
        headers: { "Authorization": `Bearer ${currentGeminiKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({
          model,
          messages: [
      ...
      if (aiRes.status === 402 || aiRes.status === 401 || aiRes.status === 403) {
        ...
        const msg = `Sua chave do Gemini foi recusada (HTTP ${aiRes.status}). Verifique a GEMINI_API_KEY e se a API Generative Language está habilitada.`;
        console.error(`[gerar-aula-livro-v2] gemini ${aiRes.status}: ${detail}`);
        return json({ error: msg, status: aiRes.status, detail }, aiRes.status);
      }
      ...
      if (!aiRes.ok) {
        ...
        console.error(`[gerar-aula-livro-v2] gemini ${aiRes.status}; model=${model}; attempt=${attempt + 1}`);
      ...
      console.warn(`[gerar-aula-livro-v2] resposta inválida; model=${model}; attempt=${attempt + 1}; ${lastDetail}`);
    }
    ...
    console.error("[gerar-aula-livro-v2]", e);
