...
DENTRO DE CADA EXEMPLO (nesta ordem):
1. "### O caso" — a situação narrada em 2 a 4 linhas.
2. "### Encaixando na lei" — TABELA markdown com 3 colunas: | Trecho da lei | O que aconteceu no caso | Encaixa? |. No trecho da lei use \`crase simples\` copiando a EXPRESSÃO EXATA do dispositivo; na última coluna use "Sim" / "Não" e o motivo em poucas palavras.
3. "### Como se aplica" — o raciocínio ligando fato e norma, sempre destacando com \`crase simples\` cada expressão legal usada e explicando ali mesmo por que ela se cumpre.
4. "### Se mudasse um detalhe" — uma variação do caso que muda o enquadramento (outro artigo, atipicidade, forma qualificada).
- Se o caso tiver etapas/prazos, acrescente uma linha do tempo em lista numerada.
- Todo trecho de lei citado no meio do texto vai em \`crase simples\`; a transcrição do dispositivo vai em "> " (citação).`;
    ...
    const geminiBody: any = {
      ...
      generationConfig: {
        ...
        maxOutputTokens: mode === 'headline'
          ? 150
          : (mode === 'mapa_mental_grafo' || mode === 'carrossel_post' || mode === 'grifo_magico' || mode === 'explicacao' || mode === 'exemplo')
            ? 8192
            : 4096,
