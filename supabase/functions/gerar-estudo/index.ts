...
type ArtigoRow = { numero: string; texto: string | null; epigrafe: string | null };
...
async function fetchArticleText(tabelaNome: string, artigoNumero: string): Promise<string> {
  ...
  const withoutOrdinal = normalized.replace(/[º°ᵒ]$/, "").trim();
  const candidates = Array.from(new Set([
    artigoNumero.trim(),
    normalized,
    withoutOrdinal,
  ...
  // Tentativa 1: igualdade exata, um candidato por vez (evita que uma única
  // consulta .in() com caracteres especiais derrube todas as variantes).
  for (const cand of candidates) {
    const { data, error } = await supabase
      ...
      .limit(1);
    if (error) {
      console.error("[gerar-estudo] consulta artigo falhou", { cand, code: error.code, message: error.message, details: error.details });
      continue;
    }
    const row = data?.[0] as ArtigoRow | undefined;
    if (row) return formatArticle(row);
  }

  // Tentativa 2: comparação tolerante a ordinal/pontuação (ex.: "2º" vs "2o" vs "2.").
  const { data: fuzzy, error: fuzzyErr } = await supabase
    .from("vade_mecum_artigos")
    .select("numero, texto, epigrafe")
    .eq("lei_id", leiId)
    .ilike("numero", `${withoutOrdinal}%`)
    .limit(20);
  if (fuzzyErr) {
    console.error("[gerar-estudo] consulta tolerante falhou", { code: fuzzyErr.code, message: fuzzyErr.message });
  }
  const simplify = (s: string) => s.toLowerCase().replace(/[º°ᵒ.\s]/g, "");
  const alvo = simplify(withoutOrdinal);
  const hit = (fuzzy as ArtigoRow[] | null)?.find((r) => simplify(r.numero) === alvo);
  if (hit) return formatArticle(hit);

  throw new Error(`Artigo não encontrado: ${tabelaNome}.${artigoNumero} (tentou: ${candidates.join(", ")})`);
}

function formatArticle(row: ArtigoRow): string {
...
${articleText}`;
    ...
    console.error("gerar-estudo error:", msg);
    // Devolve 200 com campo `error` para o app conseguir exibir a mensagem real
    // (com non-2xx o supabase-js só entrega "Edge Function returned a non-2xx status code").
