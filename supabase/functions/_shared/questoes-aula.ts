// Gera MUITAS questões (módulo compartilhado, roteado por biblioteca-ocr-mistral).
...
REGRAS:
- Meta: 15 a 20 questões em aulas normais; 20 a 25 quando a aula for extensa. Nunca menos de 15.
- Cubra TODO o conteúdo da aula: cada seção/tema relevante deve gerar pelo menos uma questão.
...
export async function handleGerarQuestoesAula(req: Request, raw: any) {
  try {
    ...
    console.log(`[gerar-questoes-aula] provedor=${PROVIDER} modelo=${MODEL}`);
    ...
    if (!aiRes.ok) {
      ...
      console.error(`[gerar-questoes-aula] gemini ${aiRes.status}: ${detail}`);
    ...
    if (questoes.length < 3) return json({ error: "IA retornou poucas questões", parsed, provider: PROVIDER, model: MODEL }, 502);
