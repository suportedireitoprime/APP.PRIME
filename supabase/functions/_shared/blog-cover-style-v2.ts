...
COMPOSITION (CRITICAL): EXACTLY CENTERED. The main figure MUST sit precisely on the horizontal centre of the canvas (dead center), bottom-aligned. DO NOT place the figure on the left. DO NOT place the figure on the right. The figure is the central axis. Supporting elements must be distributed symmetrically on both sides of the figure to keep the composition perfectly balanced. Never push the figure to a side third, never leave one half of the panel empty.

MAIN SUBJECT — ONE CUT-OUT FIGURE: a single flat vector illustrated figure, ${figure}. ${side}. The figure is a clean CUT-OUT (knockout) illustration placed on top of the colour panel: no ground, no shadow scenery, no room around it, only a soft drop shadow. It occupies roughly 60-72% of the frame height, feet/base touching the bottom edge, and is fully inside the frame (never cropped at the head or hands). It MUST be positioned perfectly in the middle of the image.
