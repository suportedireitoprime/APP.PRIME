...
export function LocalTransporteDialog({ open, onClose, origem, destino }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data, error } = await supabase.functions.invoke('local-info', {
          body: { fn: 'transporte', origem, destino: { lat: destino.lat, lng: destino.lng }, destino_nome: destino.nome },
        });
