...
import { Calendar, PlayCircle, Route as RouteIcon, CheckCircle2, Settings2, Lightbulb } from 'lucide-react';
import { haptic } from '@/lib/nativeHaptics';
import { slugify, limparTitulo, ytThumb, formatDuracao, getCapaDaArea } from '@/lib/videoaulasCatalogos';
...
import type { CatalogoAula } from './catalogoTrilhaTypes';

interface CatalogoTrilhaMapProps {
  catalogoId: string;
  titulo: string;
  aulas: CatalogoAula[];
  onBack?: () => void;
...
export const CatalogoTrilhaMap = ({
  catalogoId,
  titulo,
  aulas,
}: CatalogoTrilhaMapProps) => {
  ...
  return (
    <div className="w-full pb-32">
      <div className="sticky top-0 z-40 bg-background/95 border-b border-white/5 px-4 py-4 flex flex-col gap-3 shadow-sm">
        ...
        <div className="flex bg-white/5 rounded-full p-1 mt-1">
          <button
            ...
            className={`flex-1 py-2 text-xs font-bold rounded-full transition-all flex items-center justify-center gap-2 ${
              viewMode === 'map'
      ...
      {viewMode === 'map' ? (
        ...
        <>
          <div className="w-full overflow-x-auto hide-scrollbar px-4 py-4 border-b border-white/5 sticky top-[110px] z-30 bg-background/95">
            <div className="flex items-center gap-3">
              {nodos.map((nodo) => {
                    ...
                    return (
                      <div key={idx} className="relative flex items-center gap-4 py-3">
                        ...
                        <button
                          ...
                          className="group flex gap-4 w-full text-left bg-card border border-white/5 rounded-2xl p-2.5 hover:bg-card hover:border-white/20 transition-all active:scale-[0.98]"
                        >
