...
export function VisuaisPdfModal({ open, registro, onClose }: VisuaisPdfModalProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      cancelado = true;
      if (createdUrl) {
        // Atraso de liberação para não quebrar iframes em desmontagem
        setTimeout(() => URL.revokeObjectURL(createdUrl), 1500);
      }
      setPdfUrl(null);
    };
  }, [open, registro]);
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[120] flex flex-col bg-black/95 backdrop-blur-md isolate">
      {/* Top Header */}
