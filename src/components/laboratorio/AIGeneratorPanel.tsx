...
export default function AIGeneratorPanel() {
  ...
  // Removido TTS de teste (agora é delegado ao DynamicSceneLoader)
      ...
      <AnimatePresence>
        {previewModalOpen && (
          ...
          >
            ...
            >
              <div className="flex-1 w-full h-full relative bg-black rounded-b-3xl overflow-hidden">
                <button 
                  onClick={() => setPreviewModalOpen(false)}
                  className="absolute top-4 right-4 z-50 p-3 bg-black/60 backdrop-blur-md border border-white/10 rounded-full hover:bg-red-500 text-white transition-colors"
                >
                  <X size={20} />
                </button>
                <ErrorBoundary>
