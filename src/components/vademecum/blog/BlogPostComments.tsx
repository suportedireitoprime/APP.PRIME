...
const BlogPostComments = ({ postId, open, onClose }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        .from('blog_post_comments' as any)
  ...
  const enviar = async () => {
    ...
    setComentarios((prev) => [data as unknown as Comentario, ...prev]);
    setTexto('');
  };

  const apagar = async (id: string) => {
    const { error } = await supabase.from('blog_post_comments' as any).delete().eq('id', id);
    if (error) {
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 z-[70] w-full max-w-md bg-card border-l border-border shadow-2xl flex flex-col"
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {user ? (
                ...
                <p className="text-xs text-muted-foreground font-body">Faça login para comentar.</p>
              )}
