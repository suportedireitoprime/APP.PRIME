...
const DesktopToolsMenu = () => {
  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative w-11 h-11 rounded-xl bg-neutral-900/70 backdrop-blur border border-primary-foreground/40 hover:border-primary-foreground/70 hover:bg-neutral-900 flex items-center justify-center transition-colors group"
        aria-label="Todas as funções"
        aria-expanded={open}
        title="Todas as funções"
      >
        {open ? (
          <X className="w-5 h-5 text-primary drop-shadow-[0_1px_2px_rgba(0,0,0,0.6)]" />
        ) : (
          <LayoutGrid className="w-5 h-5 text-primary drop-shadow-[0_1px_2px_rgba(0,0,0,0.6)] group-hover:scale-110 transition-transform" />
        )}
      </button>
