...
const ArtigoPdfDoc = ({ artigos, leiNome, highlights }: { artigos: ArtigoLei[]; leiNome: string; highlights?: Record<string, any[]> }) => (
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <Text style={styles.subheader}><Text style={styles.subheader}>Exportado via Direito Prime - Vade Mecum Jurídico Profissional</Text></Text>
      {artigos.map((art, i) => (
      ...
      <Text style={styles.footer} render={({ pageNumber, totalPages }) => `Direito Prime — Página ${pageNumber} de ${totalPages}`} fixed />
    </Page>
  </Document>
);
