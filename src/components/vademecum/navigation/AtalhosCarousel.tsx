...
import JurisprudenciaSheet from '@/components/vademecum/sheets/JurisprudenciaSheet';
...
const COLOR_MAP: Record<string, string> = {
  cf88: '#FFD400',  // amarelo Constituição
  cp:   'hsl(348 78% 38%)',  // vermelho vivo
  cc:   '#3B82F6',  // azul vivo
  cpc:  '#DC2626',  // âmbar
  cpp:  '#F97316',  // laranja vivo
  ctn:  '#DC2626',  // amarelo/ouro
...
const FALLBACK_COLORS = ['#22C55E', '#3B82F6', '#EC4899', '#F97316', '#DC2626', '#14B8A6', '#8B5CF6', '#06B6D4', 'hsl(348 78% 38%)', '#A855F7'];
...
const CAT_PRINCIPAIS: CatCard[] = [
  { id: 'constituicao', label: 'CONSTITUIÇÃO', sublabel: 'CF/88', icon: Landmark, accent: '#FFD400' },
...
const CAT_OUTRAS: CatCard[] = [
  ...
  { id: 'decreto',        label: 'DECRETOS', sublabel: 'Regulamentos do Executivo', icon: Stamp, accent: 'hsl(348 78% 38%)' },
...
const AtalhosCarousel = ({ onSelect }: Props) => {
  ...
  const HEADER_CONFIG: Record<Mode, { title: string; icon: React.ElementType; desc: string; color: string }> = {
    ...
    'favoritos': { title: 'Favoritos', icon: Heart,    desc: 'Suas leis salvas para acesso rápido',        color: 'hsl(348 78% 38%)' },
  };
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-[90] bg-card border-t border-border rounded-t-3xl pb-[calc(1rem+var(--sai-bottom))] h-[88vh] flex flex-col overflow-hidden"
      >
