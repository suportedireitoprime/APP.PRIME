...
export default function ShareSheet({ open, onClose, title, text, url }: Props) {
  const encodedUrl = encodeURIComponent(url);
  const encodedText = encodeURIComponent(`${title}${text ? ` – ${text}` : ''}`);

  // wa.me e t.me abrem o app nativo se instalado, senão caem no cliente web.
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 z-[70] bg-card rounded-t-3xl border-t border-border shadow-2xl mx-auto max-w-md"
          >
            ...
            <div className="px-5 pb-6 pt-2">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-display text-lg text-foreground">Compartilhar</h3>
                <button
                  onClick={onClose}
                  aria-label="Fechar"
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                >
                  ...
                  <span className="text-[11px] text-muted-foreground font-medium">Telegram</span>
                </a>
                <button
                  onClick={copiar}
                  className="flex flex-col items-center gap-2 active:scale-95 transition-transform"
                >
                  ...
                  <span className="text-[11px] text-muted-foreground font-medium">Copiar link</span>
                </button>
                <button
                  onClick={nativeShare}
                  className="flex flex-col items-center gap-2 active:scale-95 transition-transform"
                >
                  ...
                  <span className="text-[11px] text-muted-foreground font-medium">Mais</span>
                </button>
              </div>
            </div>
          </motion.div>
