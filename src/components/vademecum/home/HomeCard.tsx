...
const HomeCardImpl = ({ icon: Icon, label, sublabel, color, delay = 0, onClick, className = '', iconClassName = '', badge, 'data-track': dataTrack, 'data-track-name': dataTrackName, 'data-track-section': dataTrackSection, solidColor = false }: HomeCardProps) => (
  <button
    ...
    className={`group relative flex h-[100px] min-h-[100px] w-full min-w-0 flex-row items-center justify-between overflow-hidden p-3.5 pr-8 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] transition-all focus-visible:outline-none text-left active:scale-[0.97] border border-white/10 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-700 hover:from-zinc-800 hover:via-zinc-700 hover:to-zinc-600 ${className}`}
  >
