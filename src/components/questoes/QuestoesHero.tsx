...
/** Painel de Questões — mesmo padrão visual do painel de Flashcards. */
      ...
      <div className="relative p-4 sm:p-5">
        ...
        <div className="relative mt-3 rounded-xl bg-black/85 text-white shadow-lg ring-1 ring-black/20">
          <div className="grid grid-cols-3 divide-x divide-white/10">
            <Metric label="Hoje" value={hoje.toLocaleString('pt-BR')} sufixo=" quest." />
            <Metric label="Respondidas" value={total.toLocaleString('pt-BR')} />
            <div className="flex flex-col items-center justify-center px-2 py-2">
              <span className="text-[9px] font-bold uppercase tracking-wider text-white/60">Acertos</span>
              <span className="mt-0.5 font-display text-base font-black leading-none text-[hsl(258_90%_78%)]">
                {acertos.toLocaleString('pt-BR')}
                <span className="text-white/50">/{disponiveis.toLocaleString('pt-BR')}</span>
              </span>
            </div>
          </div>
        </div>
