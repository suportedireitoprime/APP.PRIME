...
import { StepRow, SelecaoSheet } from './chunks';

export { StepRow, SelecaoSheet };
...
const QuestoesFiltroSheet = ({
  ...
  useEffect(() => {
    ...
    db.rpc('questoes_filtro_counts', {
      ...
      if (data) {
        ...
        try {
          sessionStorage.setItem('questoes:counts-cache', JSON.stringify(data));
        } catch {
          /* noop */
        }
      }
  ...
  const proximo = !f.segmentos.length
    ? 'segmento'
  ...
  const aplicar = () => {
    ...
    haptic.success?.();
    try {
      sessionStorage.setItem(FILTRO_KEY, JSON.stringify(f));
    } catch {
      /* noop */
    }
    onAplicar(f);
          ...
          >
            ...
            <div className="flex-1 space-y-2.5 overflow-y-auto px-4 pb-4 pt-2">
              <StepRow
                ...
                hint={
                  f.segmentos.length
                    ? f.segmentos.length === SEGMENTOS.length
                      ? 'Todos os segmentos'
                      : f.segmentos.map((s) => SEGMENTOS.find((x) => x.id === s)?.label ?? s).join(', ')
                    : 'Escolha em qual base você quer praticar'
                }
                active={proximo === 'segmento'}
                done={!!f.segmentos.length}
              ...
              <StepRow
                ...
                locked={!f.segmentos.length}
                active={proximo === 'disciplinas'}
                done={!!f.disciplinas.length}
              ...
              <StepRow
                ...
                label="Assuntos"
                hint={
                  assuntosSelecionadosPrincipais.length
                    ? `${assuntosSelecionadosPrincipais.length} selecionado(s)`
                    : 'Todos os assuntos'
                }
                locked={!f.disciplinas.length}
                active={proximo === 'assuntos'}
                done={!!f.assuntos.length}
              ...
              <StepRow
                ...
                hint={
                  f.anos.length
                    ? f.anos.length === anos.length && anos.length > 0
                      ? 'Todos os anos'
                      : f.anos.join(', ')
                    : 'Todos os anos'
                }
                  ...
                  {(['embaralhado', 'original'] as const).map((o) => (
                    <button
                      ...
                      className={cn(
                        ...
                        f.ordem === o
                          ? 'bg-hero-panel text-white shadow-md shadow-[#E11D48]/20 [text-shadow:0px_1px_2px_rgba(0,0,0,0.8)]'
              ...
              {passo === 'status' && (
                <SelecaoSheet
                  ...
                  onConfirmar={(v) =>
                    setF((p) => ({
                      ...p,
                      status: v.map((label) => STATUS.find((s) => s.label === label)?.id ?? label),
                    }))
                  }
                />
              )}
