...
const ResumosCargoHero = ({ pct = 0, total = 0, hoje = 0, meta = 100, disponiveis = 0, streak = 0 }: Props) => {
  ...
  return (
    <section
      className="relative isolate overflow-hidden border-b border-black/20 rounded-b-[36px]"
      style={{ background: 'linear-gradient(135deg, hsl(199 89% 15%) 0%, hsl(199 89% 25%) 50%, hsl(199 89% 35%) 100%)' }}
      aria-label="Seu progresso em resumos"
    >
      ...
      <div className="relative p-4 sm:p-5 pt-[calc(1rem+var(--sai-top))]">
        <div className="flex items-start gap-3">
          <div className="relative shrink-0" style={{ width: size, height: size }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="mt-0.5 text-[8px] font-bold uppercase tracking-wider text-white/70">Leitura</span>
            </div>
          </div>

          <div className="min-w-0 max-w-[58%]">
            <div className="flex items-center gap-2 mb-0.5">
              <p className="text-[10px] font-bold uppercase tracking-wider text-white/75">Seu Progresso</p>
              {streak > 0 && (
            ...
            >
              Acervo de estudos focado no seu edital.
            </p>
          </div>
        </div>
        ...
        <div className="relative mt-3 rounded-xl bg-black/85 text-white shadow-lg ring-1 ring-black/20">
          <div className="grid grid-cols-3 divide-x divide-white/10">
            <Metric label="Hoje" value={hoje.toLocaleString('pt-BR')} sufixo=" lidos" />
            <Metric label="Total Lidos" value={total.toLocaleString('pt-BR')} />
            <Metric label="Acervo" value={disponiveis.toLocaleString('pt-BR')} sufixo=" resumos" />
          </div>
        </div>
      </div>

      <NotebookText className="pointer-events-none absolute bottom-3 left-3 h-8 w-8 text-white/15" />
    </section>
  );
};
...
export default ResumosCargoHero;
