...
function MindMapNodeComponent({ data, id }: NodeProps) {
  ...
  return (
    <div
      className="relative"
      style={{
        background: isRoot ? 'hsl(0, 74%, 49%)' : 'hsl(240, 6%, 14%)',
    ...
    >
      ...
      {expanded && (
        <div style={{ marginTop: '8px', borderTop: `1px solid ${color}40`, paddingTop: '8px' }}>
          ...
          {exemplo && (
            <p style={{ fontSize: '11px', color: 'hsl(0, 74%, 55%)', margin: 0, lineHeight: '1.4', fontStyle: 'italic' }}>
              📋 {exemplo}
            </p>
          )}
        </div>
  ...
  return (
    <div className="w-full h-[75vh] rounded-xl overflow-hidden border border-border bg-background">
      ...
      >
        ...
        <MiniMap
          nodeColor={(n) => n.id === 'root' ? 'hsl(0, 74%, 49%)' : 'hsl(var(--muted))'}
