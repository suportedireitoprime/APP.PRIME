...
const LeitorFolhear = forwardRef<LeitorFolhearHandle, Props>(function LeitorFolhear(
  ...
  // Em telas largas (desktop), mostra duas páginas lado a lado como um livro real.
  const isSpread = !!dims && dims.w >= 1024;
  ...
  return (
    ...
    >
      {dims && paginas.length > 0 && (
        <HTMLFlipBook
          key={`${dims.w}x${dims.h}-${isMobilePage ? 'mobile' : isSpread ? 'spread' : 'wide'}`}
          ref={bookRef}
          width={dims.w}
          height={dims.h}
          size="stretch"
