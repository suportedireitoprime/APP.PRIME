...
const QuestoesTab = ({ temaId, questoes, loading, respostas, onRespondida, onIrProgresso, disablePersist }: Props) => {
  ...
  if (resumo.respondidas === total) {
    ...
    return (
      <div className="flex flex-col items-center gap-4 rounded-3xl border-2 border-[hsl(0_72%_52%)]/40 bg-gradient-to-b from-card to-secondary/40 p-8 text-center">
        <div
        ...
        <div className="flex w-full flex-col gap-2 pt-2 sm:flex-row">
          ...
          <button
            ...
            className="inline-flex h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[hsl(0_72%_52%)] text-[14px] font-bold text-primary-foreground transition-transform hover:scale-[1.01]"
            style={{ fontFamily: "'Barlow', system-ui, sans-serif" }}
          >
