...
const formatarParagrafosCapa = (textoRaw?: string): string[] => {
  ...
  // Se já tiver dois ou mais parágrafos divididos por quebras de linha
  const partes = limpo.split(/\n\s*\n|\n/).map((p) => p.trim()).filter(Boolean);
  if (partes.length > 1) return partes;

  // Se for um bloco contínuo, quebra em 2 parágrafos no ponto final mais próximo do meio
