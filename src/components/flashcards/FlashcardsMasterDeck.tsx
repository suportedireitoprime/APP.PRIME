...
import type { FlashcardsAreaRow } from '@/lib/flashcardsQueries';

const STORAGE_KEY = 'flashcards:master_deck_last_slug';

interface FlashcardsMasterDeckProps {
...
export const FlashcardsMasterDeck: React.FC<FlashcardsMasterDeckProps> = memo(({ areas }) => {
  ...
  const [ativo, setAtivo] = useState<number>(() => {
    try {
      ...
      if (savedSlug && areas.length > 0) {
        const found = areas.findIndex((a) => a.slug === savedSlug || a.area === savedSlug);
  ...
  useEffect(() => {
    if (areas[ativo]?.area) {
      try {
        localStorage.setItem(STORAGE_KEY, areas[ativo].area);
      } catch {}
    }
  }, [ativo, areas]);
  ...
  useEffect(() => {
    if (areas.length > 0) {
      prefetchAreaCovers(areas.map(a => ({ nome: a.area, slug: a.slug })));
    }
  }, [areas]);
  ...
  const iconInfo = useMemo(() => areaIconFor(activeArea?.slug || activeArea?.area), [activeArea?.slug, activeArea?.area]);
  const AreaIcon = iconInfo?.Icon;
  const activeAccentColor = iconInfo?.color || '#34D399';
  const activePalette = useMemo(
    () => getAreaThemePalette(activeArea?.slug || activeArea?.area),
    [activeArea?.slug, activeArea?.area]
  );

  const totalCards = activeArea?.total_cards || 0;
  const concluidas = activeArea?.compreendidos || 0;
  const pctArea = totalCards > 0 ? Math.round((concluidas / totalCards) * 100) : 0;
      ...
      <div className="relative w-full -mx-2 sm:mx-0 px-2 sm:px-0 pt-2 pb-2 flex flex-col items-center select-none overflow-visible">
        ...
        <div className="relative flex items-center justify-center w-full max-w-full h-[330px] sm:h-[390px] md:h-[420px] overflow-visible">
          ...
          <div
            tabIndex={0}
            role="region"
            aria-label={`Deck de Flashcards. Matéria atual: ${activeArea.area}. Use as setas para navegar e Enter para abrir.`}
            onKeyDown={(e) => {
              if (e.key === 'ArrowRight') { e.preventDefault(); handleNext(); }
              else if (e.key === 'ArrowLeft') { e.preventDefault(); handlePrev(); }
              else if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleOpenActive(); }
            }}
            className="relative flex items-center justify-center w-full h-full outline-none focus-visible:ring-2 focus-visible:ring-[#34D399]/50 rounded-3xl"
          >
            {areas.map((area, i) => {
              const diff = i - ativo;
              ...
              const nomeMateriaTag = area.area.replace(/^Direito\s+(do\s+|da\s+|de\s+)?/i, '').replace(/^Direitos\s+/i, '').trim();
      ...
      <div className="w-full max-w-xl mx-auto px-4 py-3.5 rounded-2xl bg-gradient-to-b from-white/[0.07] to-white/[0.02] border border-white/10 shadow-xl backdrop-blur-md space-y-2.5">
        <div className="flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 border border-white/10 shadow-sm" style={{ backgroundColor: `${activeAccentColor}22` }}>
              {AreaIcon ? <AreaIcon className="w-4 h-4" style={{ color: activeAccentColor }} strokeWidth={2.2} /> : <BookOpen className="w-4 h-4" style={{ color: activeAccentColor }} />}
            </div>
            <span className="font-semibold text-white truncate text-sm">{activeArea.area}</span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-muted-foreground text-xs">{totalCards > 0 ? `${concluidas}/${totalCards} compreendidos` : '0 cards'}</span>
            <span className="px-2 py-0.5 rounded-md text-xs font-bold text-[#050505] shadow-sm" style={{ backgroundColor: activeAccentColor }}>
              {pctArea}%
            </span>
          </div>
        </div>
        ...
        <div className="w-full bg-white/10 h-2.5 rounded-full overflow-hidden border border-white/10 relative">
          <motion.div
            initial={{ width: 0 }} animate={{ width: `${Math.min(100, Math.max(0, pctArea))}%` }} transition={{ duration: 0.5, ease: 'easeOut' }}
            className="h-full rounded-full relative"
            style={{ background: `linear-gradient(90deg, ${activePalette.primary}aa, ${activePalette.primary})`, boxShadow: `0 0 10px ${activePalette.primary}` }}
          />
        </div>
      </div>
    </div>
...
FlashcardsMasterDeck.displayName = 'FlashcardsMasterDeck';
export default FlashcardsMasterDeck;
