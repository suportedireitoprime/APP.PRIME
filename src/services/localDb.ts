...
// O backend web (jeep-sqlite + sql-wasm) trava a aba com
// "RuntimeError: memory access out of bounds", derrubando o renderer e
// fazendo os imports dinâmicos das rotas falharem (tela branca).
// Em web o SQLite local fica desativado e as operações viram no-op.
const SQLITE_ENABLED = Capacitor.isNativePlatform();

async function boot(): Promise<void> {
  if (!SQLITE_ENABLED) throw new Error('localDb indisponível no ambiente web');
