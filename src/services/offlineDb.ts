...
/** Igual ao getAprenderCache, mas devolve também a idade do registro (TTL). */
export async function getAprenderCacheEntry<T = any>(
  key: string,
): Promise<{ payload: T; updatedAt: number } | null> {
  try {
    const row = await db.aprenderCache.get(key);
    if (!row) return null;
    return { payload: JSON.parse(row.payload) as T, updatedAt: row.updatedAt ?? 0 };
  } catch { return null; }
}
