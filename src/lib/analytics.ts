/**
 * Google Analytics 4 — Direito Prime (Web)
 * ------------------------------------------------------------
...
import { nativeSetConsent, nativeSetUserId } from "./nativeAnalytics";
import { flushOfflineQueue, initTrackClickListener, trackSetUser } from "./analyticsEvents";
...
export function initAnalytics() {
  ...
  // Listener global de cliques `data-track` — precisa valer TAMBÉM no app nativo,
  // senão nenhum evento declarativo é enviado ao Firebase Analytics.
  initTrackClickListener();

  if (isNativeApp()) {
    // No app o consentimento é gerenciado pelo plugin nativo; habilita a coleta.
...
/**
 * Route change → page_view (GA4 web) + Meta PageView.
 * O `screen_view` é responsabilidade do `useScreenTracking` (web e nativo),
