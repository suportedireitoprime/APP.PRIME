/**
 * Camada única de eventos do Direito Prime.
 * ------------------------------------------------------------
 ...
 *
 * O GA4/Firebase já recebia esses eventos, mas o painel administrativo lia de
 * `app_events` — por isso o funil de áreas aparecia vazio. Deduplicamos por
 * sessão para não inflar a tabela com remontagens de componente.
 */
export function logAreaEvent(event_name: string, metadata: Record<string, unknown> = {}) {
  ...
  } catch { /* storage bloqueado: registra mesmo assim */ }
...
export const appEvents = {
  ...
  signUp: (method: Method) =>
    fanout({
      ...
      ads: "sign_up",
      params: { method, content_name: "Cadastro Direito Prime" },
      db: true,
    }),
