...
/** Buffer de depuração (?ga_debug=1) — últimos eventos disparados. */
...
const debugListeners = new Set<(events: DebugEvent[]) => void>();
...
function recordDebug(name: string, params: Record<string, unknown>) {
  if (!DEBUG) return;
  debugBuffer.unshift({ name, params, platform: getPlatform(), at: Date.now() });
  if (debugBuffer.length > 60) debugBuffer.pop();
  debugListeners.forEach((fn) => fn([...debugBuffer]));
}
...
export function track(name: string, params: Record<string, unknown> = {}) {
  ...
  if (DEBUG && typeof window !== "undefined") {
    ...
    recordDebug(name, payload.params || {});
  }
...
export function trackScreen(path: string, params: Record<string, unknown> = {}) {
  ...
  if (isNativeApp()) {
    ...
    nativeLogEvent("screen_view", cleanParams(allParams));
    void metaAppEvent("ViewContent", { content_name: screenName, content_type: "screen" });
    recordDebug("screen_view", allParams);
    return;
  }
