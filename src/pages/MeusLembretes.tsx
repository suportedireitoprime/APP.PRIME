...
import NovoLembreteMenu from '@/components/lembretes/NovoLembreteMenu';
...
const MeusLembretes = () => {
  ...
  return (
    <div className="min-h-dvh bg-background">
      ...
      <div className="p-4 pb-[calc(96px+var(--sai-bottom,0px))] max-w-2xl mx-auto space-y-3 lg:max-w-[1300px] lg:px-12 lg:py-8 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0 2xl:grid-cols-3 2xl:px-16">
        <button
          onClick={() => { setEditing(null); setMenuOpen(true); }}
          className="w-full h-14 rounded-2xl border-2 border-dashed border-primary/50 lg:col-span-full bg-primary/5 text-primary font-body font-semibold flex items-center justify-center gap-2 hover:bg-primary/10 transition"
        >
        ...
        {loading ? (
          <div className="py-16 flex justify-center lg:col-span-full"><Loader2 className="w-6 h-6 animate-spin text-primary" /></div>
        ) : rows.length === 0 ? (
          <div className="py-16 text-center space-y-3 lg:col-span-full">
      ...
      <NovoLembreteMenu
        ...
        onHorario={() => { setEditing(null); setSheetOpen(true); }}
        onLocal={() => navigate('/lembretes/local')}
      />
