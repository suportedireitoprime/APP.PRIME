...
const TRIBUNAL_UI = {
  STJ: {
    ...
    Icon: Scale,
    gradient: 'linear-gradient(160deg, hsl(22348 80% 40%) 0%, hsl(220 65% 28%) 55%, hsl(220 55% 16%) 100%)',
  ...
  STF: {
    ...
    Icon: Landmark,
    gradient: 'linear-gradient(160deg, hsl(348 80% 40%) 0%, hsl(0 65% 28%) 55%, hsl(0 55% 16%) 100%)',
    ...
    <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-end justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
          {loading && (
            <div className="flex items-center justify-center py-16 text-muted-foreground lg:col-span-full">
              <Loader2 className="w-5 h-5 animate-spin" />
            </div>
          )}
          {!loading && filtrados.length === 0 && (
            <p className="text-center text-muted-foreground py-10 text-sm lg:col-span-full">Nenhum verbete encontrado.</p>
          )}
...
function InformativosTribunalInner({ tribunal }: { tribunal: 'STJ' | 'STF' }) {
  ...
  return (
    <div className="min-h-dvh bg-background pb-16">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-3 lg:max-w-[1500px] lg:px-12 lg:py-8 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0 2xl:grid-cols-3 2xl:px-16">
        {loading && (
          <div className="flex items-center justify-center py-16 text-muted-foreground lg:col-span-full">
            <Loader2 className="w-5 h-5 animate-spin" />
          </div>
        )}
        {error && <p className="text-center text-destructive text-sm py-6 lg:col-span-full">{error}</p>}
        {!loading && !error && filtradas.length === 0 && (
          <p className="text-center text-muted-foreground py-10 text-sm lg:col-span-full">
