...
import { cascadeContainer } from '@/components/vademecum/outros/staggerVariants';
...
import RadarLegislacaoContent, { prefetchRadarData } from '@/components/vademecum/outros/RadarLegislacaoContent';
...
const CategoriaLegislacao = () => {
  ...
  // O botão voltar de qualquer lei (Constituição, Códigos, Estatutos, Leis
  // Ordinárias, Lei Penal Especial, Jurisprudência, etc.) sempre retorna para
  // o Vade Mecum, que é a origem da navegação de leis.
  const goBack = useCallback(() => {
    navigate('/vade-mecum');
  }, [navigate]);

  // Prefetch o Vade Mecum (destino do "Voltar") para a volta ser instantânea.
  useEffect(() => {
  ...
  const [openModInfo, setOpenModInfo] = useState<import('@/components/vademecum/artigo/ArtigoBottomSheet').ModificationInfo | null>(null);
  ...
  if ((leiSlugParam || tipo === 'constituicao') && !selectedLeiId) {
    ...
    return (
      <div className="theme-vademecum min-h-dvh bg-background flex items-center justify-center">
        <Loader2 className="w-8 h-8 text-primary animate-spin" />
        <PremiumGate open={showPremiumGate} onClose={() => setShowPremiumGate(false)} feature={premiumGateFeature} description={premiumGateDesc} />
      </div>
    );
