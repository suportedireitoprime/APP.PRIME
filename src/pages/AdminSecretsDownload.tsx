...
const SECRET_DETAILS: Record<string, string> = {
  ...
  APPLE_BUNDLE_ID: 'Bundle Identifier do app iOS (ex: br.com.direitoprime.app).',
...
export default function AdminSecretsDownload() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (error) {
        let details = error.message || 'Falha na sincronização';
        ...
        if (errorContext) {
          try {
            ...
            const responseText = await errorContext.clone().text().catch(() => '');
        ...
        {showGuide && platform === 'android' && (
          <div className="space-y-4 text-sm pt-2 border-t border-border">
            ...
            <Step n={1} title="Preparar o projeto (Capacitor)">
              Instale <code className="text-xs bg-muted/40 px-1 rounded">@capacitor/core @capacitor/android @capacitor/cli</code> e rode <code className="text-xs bg-muted/40 px-1 rounded">npx cap init</code>. Configure <code className="text-xs bg-muted/40 px-1 rounded">appId</code> (ex: <code className="text-xs bg-muted/40 px-1 rounded">br.com.direitoprime.app</code>) em <code className="text-xs bg-muted/40 px-1 rounded">capacitor.config.ts</code>.
        ...
        {showGuide && platform === 'apple' && (
          <div className="space-y-4 text-sm pt-2 border-t border-border">
            ...
            <Step n={1} title="Conta Apple Developer + Bundle ID">
              Ative a conta Apple Developer (US$ 99/ano). Em <b>App Store Connect</b>, registre o <b>Bundle ID</b> do app (ex: <code className="text-xs bg-muted/40 px-1 rounded">br.com.direitoprime.app</code>) e salve em <code>APPLE_BUNDLE_ID</code>. O <b>Team ID</b> aparece no canto superior direito — salve em <code>APPLE_TEAM_ID</code>.
