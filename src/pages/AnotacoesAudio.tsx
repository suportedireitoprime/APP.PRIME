...
export default function AnotacoesAudio() {
  ...
  return (
    <div className="min-h-dvh bg-background">
      ...
      <div className="mx-auto max-w-2xl px-4 pt-4 pb-[calc(7rem+var(--sai-bottom,env(safe-area-inset-bottom,0px)))] lg:max-w-[1200px] lg:px-12 lg:pt-8">
      ...
      {isWA && (
        <div className="rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-4 text-sm">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Se não vir o Direito Prime, use o botão abaixo pra escolher o arquivo manualmente
...
function Lista({ soPendentes = false }: { soPendentes?: boolean }) {
  ...
  const openChat = (r: Recording) => {
    ...
    setChatRec(r);
    setChatHistory([{ role: 'assistant', content: 'Olá! Pergunte algo sobre essa aula e eu te ajudarei a revisar.' }]);
  };
        ...
        {chatRec && (
          ...
          >
            <div className="flex items-center p-4 border-b border-border bg-card shadow-sm">
              <Button variant="ghost" size="icon" onClick={() => setChatRec(null)} className="mr-3">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/20">
              {chatHistory.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-tr-sm' : 'bg-card border border-border text-foreground rounded-tl-sm'}`}>
                    {msg.content}
                  </div>
                </div>
              ))}
            ...
            <div className="p-4 border-t border-border bg-card pb-safe">
              <div className="flex gap-2">
                ...
                />
                <Button size="icon" className="rounded-full shrink-0" onClick={sendChatMessage} disabled={chatBusy || !chatInput.trim()}>
