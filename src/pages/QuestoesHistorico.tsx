...
export default function QuestoesHistorico() {
  ...
  const flatItens = useMemo(() => {
    const flat: Array<{ type: 'header'; chave: string } | { type: 'sessao'; sessao: SessaoHistorico }> = [];
    Object.entries(agrupado).forEach(([chave, itens]) => {
      flat.push({ type: 'header', chave });
      itens.forEach(sessao => flat.push({ type: 'sessao', sessao }));
    });
    return flat;
  }, [agrupado]);
        ...
        {sessoes.length === 0 ? (
          ...
          <div ref={listRef} style={{ height: `${virtualizer.getTotalSize()}px`, width: '100%', position: 'relative' }}>
            {virtualizer.getVirtualItems().map((virtualItem) => {
              const item = flatItens[virtualItem.index];
              ...
              if (item.type === 'header') {
                return (
                  <div
                    ...
                    className="flex items-end gap-2 px-1 pb-2 pt-2"
                  >
                    <Calendar className="w-4 h-4 text-primary" />
                    <h3 className="text-sm font-extrabold text-foreground uppercase tracking-wider">{item.chave}</h3>
                  </div>
                );
              }
              ...
              return (
                ...
                >
                  <motion.div 
                    ...
                    className="group relative flex flex-col gap-3 rounded-2xl border border-border/80 bg-card p-4 transition-colors hover:border-primary/50 hover:shadow-md cursor-pointer focus-visible:outline-none h-full"
                  >
