...
export default function AdminPesquisasProntas() {
  ...
  async function runIndex(tribunal: "stf" | "stj") {
    setPhase(tribunal);
    try {
      const { error } = await supabaseCloud.functions.invoke("pesquisas-prontas-index", { body: { fn: tribunal } });
  ...
  async function runBackfill(tribunal?: "STF" | "STJ") {
    ...
    try {
      while (true) {
        const { data, error } = await supabaseCloud.functions.invoke("pesquisas-prontas-index", {
          body: { fn: "backfill", batch: 5, tribunal },
        });
        if (error) throw error;
        const r = data as any;
