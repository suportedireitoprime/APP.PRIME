  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="w-full flex flex-col pb-32">
      ...
      <div className="pt-4 px-4">
        {loading ? (
          <div className="flex justify-center py-10"><div className="w-6 h-6 border-2 border-success border-t-transparent rounded-full animate-spin" /></div>
        ) : (
          <div className="space-y-3">
            <motion.button
              onClick={() => { haptic.selection(); onSelect('Todas as Áreas'); }}
              className="w-full flex items-center justify-between p-4 rounded-3xl border border-success/40 bg-success/5 shadow-sm hover:border-success transition-all active:scale-[0.98]"
            >
            ...
            {areas.map(area => (
              <button onClick={() => { haptic.selection(); onSelect(area); }} className="w-full rounded-3xl border border-border/40 bg-card/60 backdrop-blur-md shadow-sm p-4 flex items-center justify-between hover:border-success/50 transition-all active:scale-[0.98]">
                <div className="flex items-center gap-4">
          ...
          <div className="space-y-3">
            ...
            {temas.map(tema => (
              <motion.button
                ...
                className="w-full flex items-center justify-between p-4 rounded-3xl border border-border/40 bg-card/60 backdrop-blur-md shadow-sm hover:border-success/50 transition-all active:scale-[0.98]"
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-success/10 rounded-full flex items-center justify-center shrink-0">
                    <Layers className="w-6 h-6 text-success" />
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="w-full flex flex-col pt-4 px-4 pb-32">
      ...
      <div className="text-center mb-8">
          ...
          {opcoesDias.map(num => (
            <button
              ...
              className={`p-3 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-1 ${
                dias === num ? 'border-success bg-success/10 shadow-sm' : 'border-border/50 bg-card/60 hover:border-success/50'
              }`}
            >
              <span className={`text-xl font-black ${dias === num ? 'text-success' : 'text-foreground'}`}>{num}</span>
              <span className={`text-[10px] uppercase font-bold ${dias === num ? 'text-success' : 'text-muted-foreground'}`}>Dias</span>
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-8">
          {opcoesCards.map(num => (
            <button
              ...
              className={`p-3 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-1 ${
                cards === num ? 'border-success bg-success/10 shadow-sm' : 'border-border/50 bg-card/60 hover:border-success/50'
              }`}
            >
              <span className={`text-xl font-black ${cards === num ? 'text-success' : 'text-foreground'}`}>{num}</span>
              <span className={`text-[10px] uppercase font-bold ${cards === num ? 'text-success' : 'text-muted-foreground'}`}>Cards</span>
            </button>
          ))}
        </div>
        ...
        <button 
          ...
          className="w-full bg-success hover:bg-success text-white font-bold text-base h-14 rounded-2xl shadow-lg shadow-success/20 transition-all active:scale-[0.98] mt-4 flex items-center justify-center gap-2"
        >
          Criar Trilha
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="w-16 h-16 relative">
            <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 36 36">
              ...
              <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="hsl(var(--success))" strokeWidth="3" strokeDasharray={`${(trilha.diasConcluidos.length / trilha.diasMeta) * 100}, 100`} />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center text-success font-bold text-xs">
            ...
            return (
              <div key={dia} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                <div className="flex items-center justify-center w-12 h-12 rounded-full border-4 border-background bg-card shadow-sm z-10 md:mx-auto shrink-0 relative transition-colors">
                  {isCompleted ? (
                    <div className="w-full h-full bg-success text-white rounded-full flex items-center justify-center shadow-lg shadow-success/20">
                      <CheckCircle2 className="w-6 h-6" />
                    </div>
                  ) : isAccessible ? (
                    <div className="w-full h-full bg-background border-2 border-success text-success rounded-full flex items-center justify-center shadow-lg shadow-success/10">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${
                          isCompleted ? 'bg-destructive/10 text-destructive' : 'bg-success/10 text-success hover:bg-success hover:text-white'
                        }`}
                      >
          ...
          <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="w-full flex flex-col pb-12">
            ...
            <div className="px-4 mt-6">
              <button 
                ...
                className="w-full bg-card hover:bg-card/80 border border-success/30 p-4 rounded-3xl flex items-center justify-between transition-all active:scale-[0.98] shadow-sm mb-8"
              >
                <div className="flex items-center gap-4">
