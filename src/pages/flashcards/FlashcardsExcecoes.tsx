...
const EXCECOES_TIPOS = [
  { id: 'excecoes_penais', label: 'Exceções Penais', desc: 'Direito Penal e Processual', icon: Flame, color: 'text-rose-500', keywords: ['exceção', 'salvo', 'exceto', 'penal', 'processo penal'] },
  { id: 'excecoes_civis', label: 'Exceções Civis', desc: 'Direito Civil e Processual', icon: Flame, color: 'text-blue-500', keywords: ['exceção', 'salvo', 'exceto', 'civil', 'processo civil'] },
  { id: 'excecoes_gerais', label: 'Exceções em Geral', desc: 'Regras e ressalvas', icon: Quote, color: 'text-emerald-500', keywords: ['exceção', 'salvo', 'exceto', 'regra geral', 'ressalva'] }
];

export default function FlashcardsExcecoes() {
  ...
  // States
  const [tipoSelecionado, setTipoSelecionado] = useState<typeof EXCECOES_TIPOS[0] | null>(null);
  ...
  useEffect(() => {
    document.title = 'Flashcards Exceções | Vade Mecum PRIME';
  }, []);
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden bg-background pb-28 lg:pb-12">
      <PageHeader 
        title="Exceções & Regras"
        subtitle="Explore as exceções que costumam cair em provas"
        onBack={() => {
