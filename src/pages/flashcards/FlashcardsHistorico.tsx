import { useMemo, useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { PageHeader } from '@/components/vademecum/navigation/PageHeader';
import { getFlashcardsSessoes, removeFlashcardsSessao, type FlashcardsSessaoHistorico } from '@/lib/flashcardsSessoes';
import { haptic } from '@/lib/nativeHaptics';
import { Layers, PlayCircle, Trash2, Calendar, Target, CheckCircle2 } from 'lucide-react';
import { format, isToday, isYesterday, parseISO } from 'date-fns';
import { ptBR } from 'date-fns/locale';

export default function FlashcardsHistorico() {
  const navigate = useNavigate();
  const [sessoes, setSessoes] = useState<FlashcardsSessaoHistorico[]>([]);

  useEffect(() => {
    setSessoes(getFlashcardsSessoes());
    document.title = 'Histórico | Flashcards';
  }, []);

  const handleExcluir = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    haptic.selection();
    removeFlashcardsSessao(id);
    setSessoes(getFlashcardsSessoes());
  };

  const handleContinuar = (sessao: FlashcardsSessaoHistorico) => {
    haptic.selection();
    navigate(`/flashcards/estudar?${sessao.queryString}`);
  };

  // Agrupar sessões por data
  const agrupado = useMemo(() => {
    const grupos: Record<string, FlashcardsSessaoHistorico[]> = {};
    for (const s of sessoes) {
      if (!s.dataInicio) continue;
      const data = parseISO(s.dataInicio);
      let chave = format(data, "dd 'de' MMMM", { locale: ptBR });
      if (isToday(data)) chave = 'Hoje';
      else if (isYesterday(data)) chave = 'Ontem';

      if (!grupos[chave]) grupos[chave] = [];
      grupos[chave].push(s);
    }
    return grupos;
  }, [sessoes]);

  return (
    <div className="theme-questoes min-h-dvh overflow-x-hidden bg-background pb-[calc(1rem+var(--sai-bottom))]">
      <PageHeader title="Histórico de Sessões" onBack={() => navigate('/flashcards')} />

      <div className="mx-auto w-full max-w-3xl px-4 py-6 space-y-8">
        {sessoes.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-20 opacity-60">
            <Layers className="w-16 h-16 mb-4" />
            <p className="text-lg font-bold">Nenhum histórico</p>
            <p className="text-sm mt-1 max-w-[250px]">
              Suas sessões recentes de Flashcards aparecerão aqui.
            </p>
          </div>
        ) : (
          Object.entries(agrupado).map(([chave, itens]) => (
            <div key={chave} className="space-y-3">
              <div className="flex items-center gap-2 px-1">
                <Calendar className="w-4 h-4 text-primary" />
                <h3 className="text-sm font-extrabold text-foreground uppercase tracking-wider">{chave}</h3>
              </div>

              <div className="space-y-3">
                {itens.map((sessao) => {
                  const respondidas = sessao.cardsRevisados || 0;
                  const total = sessao.totalCards || 0;
                  const finalizada = respondidas >= total && total > 0;
                  const pct = total > 0 ? Math.round((respondidas / total) * 100) : 0;
                  
                  return (
                    <div 
                      key={sessao.id}
                      onClick={() => handleContinuar(sessao)}
                      className="group relative flex flex-col gap-3 rounded-2xl border border-border/80 bg-card p-4 transition-all hover:border-[#36AF85]/50 hover:shadow-md cursor-pointer active:scale-[0.98]"
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="font-bold text-foreground text-base leading-tight pr-8">
                            {sessao.filtroAplicado}
                          </p>
                          <p className="text-[11px] text-muted-foreground mt-1 font-semibold flex items-center gap-1.5">
                            <Target className="w-3 h-3" />
                            Progresso: {respondidas} de {total} ({pct}%)
                          </p>
                        </div>
                        <button 
                          onClick={(e) => handleExcluir(sessao.id, e)}
                          className="absolute right-4 top-4 p-1.5 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 rounded-full transition-colors"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>

                      {/* Barra de progresso visual */}
                      <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-[#36AF85] transition-all duration-500" 
                          style={{ width: `${pct}%` }} 
                        />
                      </div>

                      <div className="flex justify-between items-center pt-1 border-t border-border/50">
                        <span className="text-[10px] text-muted-foreground font-medium">
                          {sessao.dataUltimoAcesso ? format(parseISO(sessao.dataUltimoAcesso), "HH:mm", { locale: ptBR }) : ''}
                        </span>
                        
                        <div className="flex items-center gap-1.5 font-bold text-xs text-[#36AF85] group-hover:scale-105 transition-transform">
                          {finalizada ? (
                            <>
                              <CheckCircle2 className="w-4 h-4" /> Refazer
                            </>
                          ) : (
                            <>
                              <PlayCircle className="w-4 h-4" /> Continuar
                            </>
                          )}
                        </div>
                      </div>
                    </div>
                  );
                })}
              </div>
            </div>
          ))
        )}
      </div>
    </div>
  );
}
