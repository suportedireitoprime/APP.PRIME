...
import { ChevronRight, Search, Sparkles, Scale, BookOpen, Clock, FileText, Quote, Library, GraduationCap, Layers } from 'lucide-react';
...
const CLASSIFICACOES_TIPOS = [
  { id: 'classificacao_crimes', label: 'Classificação de Crimes', desc: 'Direito Penal', icon: Layers, color: 'text-rose-500', keywords: ['classificação', 'espécies', 'crime', 'penal'] },
  { id: 'classificacao_civil', label: 'Classificações Civis', desc: 'Direito Civil', icon: Layers, color: 'text-blue-500', keywords: ['classificação', 'espécies', 'civil'] },
  { id: 'classificacao_geral', label: 'Elementos e Requisitos', desc: 'Geral', icon: Quote, color: 'text-amber-500', keywords: ['requisitos', 'elementos', 'características'] }
];

export default function FlashcardsClassificacoes() {
  ...
  // States
  const [tipoSelecionado, setTipoSelecionado] = useState<typeof CLASSIFICACOES_TIPOS[0] | null>(null);
  ...
  useEffect(() => {
    document.title = 'Flashcards Classificações | Vade Mecum PRIME';
  }, []);
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden bg-background pb-28 lg:pb-12">
      <PageHeader 
        ...
        subtitle="Explore as classificações e espécies no direito"
        onBack={() => {
