...
import { ChevronRight, Search, Sparkles, Scale, BookOpen, Clock, FileText, Quote, Library, GraduationCap } from 'lucide-react';
...
const JURISTAS_TIPOS = [
  { id: 'doutrina_penal', label: 'Doutrina Penal', desc: 'Penalistas e Processualistas', icon: Library, color: 'text-rose-500', keywords: ['doutrina penal', 'penalista', 'nucci', 'capez', 'bitencourt', 'sanches', 'mirabete', 'zaffaroni', 'lopes jr', 'pacelli', 'távora'] },
  { id: 'doutrina_civil', label: 'Doutrina Civil', desc: 'Civilistas', icon: GraduationCap, color: 'text-blue-500', keywords: ['doutrina civil', 'civilista', 'tartuce', 'rosenvald', 'farias', 'gagliano', 'gonçalves', 'venosa', 'diniz', 'tepedino', 'marinoni', 'didier', 'neves', 'theodoro'] },
  { id: 'doutrina_publico', label: 'Direito Público', desc: 'Constitucional e Administrativo', icon: Quote, color: 'text-emerald-500', keywords: ['doutrina constitucional', 'administrativista', 'barroso', 'mendes', 'canotilho', 'carvalho', 'meirelles', 'di pietro', 'carvalho filho', 'alexandrino', 'moraes', 'novelino'] },
  { id: 'geral', label: 'Juristas em Geral', desc: 'Outros Autores', icon: BookOpen, color: 'text-purple-500', keywords: ['doutrina', 'autor', 'jurista', 'entendimento doutrinário', 'segundo a doutrina', 'posição majoritária'] }
];

export default function FlashcardsJuristas() {
  ...
  // States
  const [tipoSelecionado, setTipoSelecionado] = useState<typeof JURISTAS_TIPOS[0] | null>(null);
  ...
  useEffect(() => {
    document.title = 'Flashcards Juristas | Vade Mecum PRIME';
  }, []);
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden bg-background pb-28 lg:pb-12">
      <PageHeader 
        title="Juristas & Doutrina" 
        subtitle="Explore as visões doutrinárias e os autores"
        onBack={() => {
