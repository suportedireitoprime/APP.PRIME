...
const FILOSOFOS_TIPOS = [
  { id: 'filosofia', label: 'Filosofia do Direito', desc: 'Pensadores e Teorias Filosóficas', icon: Library, color: 'text-amber-500', keywords: ['filosofia', 'filósofo', 'platão', 'aristóteles', 'sócrates', 'kant', 'hegel', 'habermas', 'rawls', 'dworkin', 'alexy'] },
  { id: 'sociologia', label: 'Sociologia Jurídica', desc: 'Aspectos sociológicos e pensadores', icon: GraduationCap, color: 'text-blue-500', keywords: ['sociologia', 'sociólogo', 'foucault', 'weber', 'marx', 'durkheim'] },
  { id: 'tgd', label: 'Teoria Geral do Direito', desc: 'Conceitos e teóricos fundamentais', icon: Quote, color: 'text-purple-500', keywords: ['teoria geral', 'tgd', 'kelsen', 'radbruch', 'reale', 'bobbio'] }
];

export default function FlashcardsFilosofos() {
  ...
  // States
  const [tipoSelecionado, setTipoSelecionado] = useState<typeof FILOSOFOS_TIPOS[0] | null>(null);
  ...
  useEffect(() => {
    document.title = 'Flashcards Filósofos | Vade Mecum PRIME';
  }, []);
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden bg-background pb-28 lg:pb-12">
      <PageHeader 
        title="Filósofos & Pensadores"
        subtitle="Explore as teorias que fundamentam o direito"
        onBack={() => {
