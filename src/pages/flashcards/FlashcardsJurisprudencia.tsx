...
const JURIS_TIPOS = [
  { id: 'sumula', label: 'Súmulas', desc: 'Súmulas e Súmulas Vinculantes', icon: Landmark, color: 'text-amber-500', keywords: ['súmula', 'sumula'] },
  { id: 'informativo', label: 'Informativos', desc: 'Informativos de Jurisprudência', icon: FileText, color: 'text-blue-500', keywords: ['informativo', 'info'] },
  { id: 'tese', label: 'Teses', desc: 'Jurisprudência em Teses e Repercussão Geral', icon: MessageSquareText, color: 'text-purple-500', keywords: ['tese', 'repercussão', 'repetitivo'] }
];

export default function FlashcardsJurisprudencia() {
  ...
  // States
  const [tipoSelecionado, setTipoSelecionado] = useState<typeof JURIS_TIPOS[0] | null>(null);
  ...
  useEffect(() => {
    document.title = 'Flashcards Jurisprudência | Vade Mecum PRIME';
  }, []);
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden bg-background pb-28 lg:pb-12">
      <PageHeader 
        title={tipoSelecionado ? tipoSelecionado.label : "Jurisprudência"} 
        onBack={() => {
        ...
        {!tipoSelecionado ? (
          <>
            <div className="mb-6">
              <h2 className="text-xl sm:text-2xl font-extrabold text-foreground tracking-tight flex items-center gap-2">
                <Gavel className="w-6 h-6 text-[#36AF85]" />
                Tipos de Jurisprudência
              </h2>
              <p className="mt-1 text-sm text-muted-foreground">
                Selecione qual formato de jurisprudência você deseja estudar agora.
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {JURIS_TIPOS.map(tipo => (
                <button
                  key={tipo.id}
                  onClick={() => {
                    haptic.selection();
          ...
          <>
            <div className="mb-6">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Escolha o tema para gerar os flashcards.
              </p>
            </div>
          ...
          <div className="px-6 pb-8 overflow-y-auto flex-1">
            ...
            {passo === 2 && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-3">
                  {[
                    { id: 'sequencial', label: 'Ordem Sequencial', desc: 'Respeita a ordem dos informativos/súmulas', icon: FileText },
