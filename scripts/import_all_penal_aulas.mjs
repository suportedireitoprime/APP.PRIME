...
async function run() {
  console.log('🚀 Iniciando substituição completa das aulas de Direito Penal...');
  ...
  console.log(`Encontrados ${modulos.length} módulos no banco de dados.`);

  // 3. Apagar TODAS as aulas existentes dos módulos de Direito Penal
  const moduloIds = modulos.map(m => m.id);
  console.log(`🗑️ Apagando todas as aulas existentes dos ${moduloIds.length} módulos de Penal...`);
  ...
  if (delErr) {
    console.error('Erro ao deletar aulas antigas:', delErr);
    return;
  }
  console.log('✅ Aulas antigas removidas com sucesso.');

  // 4. Ler abas da planilha
  const wbXml = fs.readFileSync('penal_extracted/xl/workbook.xml', 'utf8');
  ...
  for (let sIdx = 0; sIdx < contentSheets.length; sIdx++) {
    ...
    const sheetFile = `penal_extracted/xl/worksheets/sheet${sIdx + 2}.xml`;
